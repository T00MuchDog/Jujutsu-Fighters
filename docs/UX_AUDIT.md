...
| ID | Priority | Improvement | Area and UX goals | How to accomplish it | Evidence |
|---|---|---|---|---|---|
| S01 | P0 | Give every fighter a recommended legal move set. | Efficiency, onboarding, cognitive load | Seed a recommended set or generate one deterministically from legal moves. Present `PLAY RECOMMENDED`, `CUSTOMIZE`, and `RANDOMIZE`. Let new players start without authoring both player and CPU sets. | Empty drafts block confirmation (`CharacterSelectScreen.java:601-607,1732-1746`), and setup guidance is buried in the learned-moves panel (`CharacterSelectScreen.java:1861-1865`). |
...
| B05 | P0 | Show targets directly on queued actions. | Plan review, cognitive load | Add target name/short ID, ally/enemy marker, multi-target count, ordered-pair slots, and an incomplete/stale warning badge. Provide full details on focus/hover. | Targets are stored separately (`PlanningPanel.java:581-590`), while `ActionSegmentView` renders only move name and type rail (`ActionSegmentView.java:14-18,50-67`). |
| B06 | P0 | Add inspectable combat statuses and durations. | Decision support, cognitive load | Render compact status icons by each HUD. On focus/hover/click, show name, effect, source, duration, stacks, and restrictions using text and shape as well as color. Keep inspection available during planning and playback. | `CombatantPanel` renders sprite/name/HP/CE only (`CombatantPanel.java:20-34,277-315`); no generic status inspection is drawn by `BattleScreen.java:985-1007`. |
